...
%>
	<form method="post" action="booking/b2.jsp">	
...
%>			
		<td  align=center><font color=red><%=calDate%>(<%=calE%>)</td>	
<%			
		}else{
%>		<td  align=center><%=calDate%>(<%=calE%>)</td>			
<%
		}		
		//예약 가능할 때와 안될때 구분해주기
		if(vipStatus.equals("SELECT")){
%>
		<td  align=center><a href ="b2.jsp?date1=<%=calDate%>&roomCnt=1"><%=vipStatus%></a></td>
<%						
		}else{
%>
		<td  align=center><%=vipStatus%></a></td>
<%		}
		if(cabanaStatus.equals("SELECT")){
%>		
		<td  align=center><a href ="b2.jsp?date1=<%=calDate%>&roomCnt=2"><%=cabanaStatus%></a></td>
<%						
		}else{
%>
		<td  align=center><%=cabanaStatus%></td>
<%		}
		if(suiteStatus.equals("SELECT")){
%>			
		<td  align=center><a href ="b2.jsp?date1=<%=calDate%>&roomCnt=3"><%=suiteStatus%></a></td>
		
<%						
		}else{
%>
		<td  align=center><%=suiteStatus%></a></td>	
<%		}
%>
	</tr>	
