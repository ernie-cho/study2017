...
;(function($, window, document, undefined) {
	...
	function userSelect(e,main,month,year){
		...
		if( sel1 === null && !isDisabled ) {
			...
			_id('out1').innerHTML = '<br>출발일 : ' + year + '년' + month + '월' + e.innerText + '일';
		...
		// second doesnt exist
			...
			_for(td, function(e){
				...
				if( s1i < s2i && go ){
					...
					_id('out2').innerHTML = '<br>도착일 : ' + year + '년' + month + '월' + e.innerText + '일';
	...
	function getMonth(year,month){
		...
		monthInformation.call(m, year, month);
			...
			str += '<table>';
			str += '<thead><tr><td><font color=red>일</font></td><td>월</td><td>화</td><td>수</td><td>목</td><td>금</td><td>토</td></tr></thead><tbody>';
...
getMonth(month, year);
$('#month').text(year  + ' - ' +monthArr[month-1]); // set month text
...
function bind(month,year){
	...
	$('#disp').on('click', 'div', function(){
		...
		$('table').remove();
		getMonth(month,year);
		$('#month').text(year  + ' - ' +monthArr[month-1]);
	})
